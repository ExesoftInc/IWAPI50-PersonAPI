...
namespace PersonAPI.Services {
    ...
    public class PhoneNumberTypeBuilder : IPhoneNumberTypeBuilder {
        ...
        public async Task<BuilderResponse> GetPhoneNumberType_ByPhoneNumberTypeID(int phoneNumberTypeID) {
            var query = Search(_entities.PhoneNumberTypes, x => x.PhoneNumberTypeID == phoneNumberTypeID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; PhoneNumberType with phoneNumberTypeID = '{phoneNumberTypeID}' doesn't exist." }); 
            }
        }
