...
namespace PersonAPI.Services {
    ...
    public class AddressBuilder : IAddressBuilder {
        ...
        public async Task<BuilderResponse> GetAddress_ByAddressID(int addressID) {
            var query = Search(_entities.Addresses, x => x.AddressID == addressID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; Address with addressID = '{addressID}' doesn't exist." }); 
            }
        }
