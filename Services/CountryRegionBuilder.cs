...
namespace PersonAPI.Services {
    ...
    public class CountryRegionBuilder : ICountryRegionBuilder {
        ...
        public async Task<BuilderResponse> GetCountryRegion_ByCountryRegionCode(string countryRegionCode) {
            var query = Search(_entities.CountryRegions, x => x.CountryRegionCode == countryRegionCode).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; CountryRegion with countryRegionCode = '{countryRegionCode}' doesn't exist." }); 
            }
        }
