...
namespace PersonAPI.Services {
    ...
    public class EmailAddressBuilder : IEmailAddressBuilder {
        ...
        public EmailAddressBuilder(EntitiesContext context, IMapper mapper, ILoggerManager logger) {
            _entities = context;
            _mapper = mapper;
            _logger = logger;
        }
        ...
        private Expression<Func<EmailAddress, EmailAddressModel>>  ProjectToModel {
            get {
                return entity => _mapper.Map<EmailAddressModel>(entity);
            }
        }
        
        public IQueryable<EmailAddressModel> GetEmailAddresses() {
            return _entities.EmailAddresses.Select(ProjectToModel);
        }
        ...
        public async Task<BuilderResponse> GetEmailAddress_ByBusinessEntityIDEmailAddressID(int businessEntityID, int emailAddressID) {
          var query = await Search(_entities.EmailAddresses, x => x.BusinessEntityID == businessEntityID&& x.EmailAddressID == emailAddressID).Select(ProjectToModel)?.ToListAsync();
           ...
           return new BuilderResponse { RequestMessage = $"Record Not Found; EmailAddress with businessEntityID, emailAddressID = '{businessEntityID}', '{emailAddressID}' doesn't exist." }; 
            }
        }
        
        public IQueryable<EmailAddressModel> GetEmailAddress_ByBusinessEntityID(int businessEntityID) {

          var query = Search(_entities.EmailAddresses, x => x.BusinessEntityID == businessEntityID).Select(ProjectToModel);
        ...
        public async Task<BuilderResponse> AddEmailAddress(EmailAddressModel model) {
           ...
           try
           {
                 var entity = _mapper.Map<EmailAddress>(model);
        ...
        public async Task<BuilderResponse> UpdateEmailAddress(EmailAddressModel model) {
          ...
          var query = Search(_entities.EmailAddresses, x =>  x.BusinessEntityID == model.BusinessEntityID && x.EmailAddressID == model.EmailAddressID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("EmailAddress with _businessEntityID, _emailAddressID = '{0}', '{1}' doesn't exist.",model.BusinessEntityID, model.EmailAddressID)}; 
            }
            var matchBusinessEntityID = _entities.People.Where(x => x.BusinessEntityID.Equals(model.BusinessEntityID));
            if (!matchBusinessEntityID.Any()) {
              return new BuilderResponse{ RequestMessage = "Foreign Key Violation; " + nameof(model.BusinessEntityID) + string.Format("EmailAddress with BusinessEntityID = '{0}' doesn't exist.", model.BusinessEntityID)}; 
            }
        ...
        public async Task<BuilderResponse> DeleteEmailAddress(int businessEntityID, int emailAddressID) {
          var query = Search(_entities.EmailAddresses, x => x.BusinessEntityID == businessEntityID&& x.EmailAddressID == emailAddressID);
            if (!query.Any()) {
              return new BuilderResponse { RequestMessage = "Record Not Found; " + string.Format("EmailAddress with _businessEntityID, _emailAddressID = '{0}', '{1}' doesn't exist.",businessEntityID, emailAddressID)}; 
            }
