...
namespace PersonAPI.Services {
    ...
    public class EmailAddressBuilder : IEmailAddressBuilder {
        ...
        public async Task<BuilderResponse> GetEmailAddress_ByBusinessEntityIDEmailAddressID(int businessEntityID, int emailAddressID) {
            var query = await Search(_entities.EmailAddresses, x => x.BusinessEntityID == businessEntityID&& x.EmailAddressID == emailAddressID).Select(ProjectToModel)?.ToListAsync();
            if (query.Any()) {
                return new BuilderResponse{ Model = query.Single() }; 
            }
            else {
                return new BuilderResponse { ValidationMessage = $"Record Not Found; EmailAddress with businessEntityID, emailAddressID = '{businessEntityID}', '{emailAddressID}' doesn't exist." }; 
            }
        }
