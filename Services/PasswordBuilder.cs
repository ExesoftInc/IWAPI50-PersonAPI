...
namespace PersonAPI.Services {
    ...
    public class PasswordBuilder : IPasswordBuilder {
        ...
        public async Task<BuilderResponse> GetPassword_ByBusinessEntityID(int businessEntityID) {
            var query = Search(_entities.Passwords, x => x.BusinessEntityID == businessEntityID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; Password with businessEntityID = '{businessEntityID}' doesn't exist." }); 
            }
        }
