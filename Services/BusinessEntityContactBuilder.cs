...
namespace PersonAPI.Services {
    ...
    public class BusinessEntityContactBuilder : IBusinessEntityContactBuilder {
        ...
        public async Task<BuilderResponse> GetBusinessEntityContact_ByBusinessEntityIDPersonIDContactTypeID(int businessEntityID, int personID, int contactTypeID) {
            var query = Search(_entities.BusinessEntityContacts, x => x.BusinessEntityID == businessEntityID&& x.PersonID == personID&& x.ContactTypeID == contactTypeID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; BusinessEntityContact with businessEntityID, personID, contactTypeID = '{businessEntityID}', '{personID}', '{contactTypeID}' doesn't exist." }); 
            }
        }
