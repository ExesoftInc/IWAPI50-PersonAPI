...
namespace PersonAPI.Services {
    ...
    public class AddressTypeBuilder : IAddressTypeBuilder {
        ...
        public async Task<BuilderResponse> GetAddressType_ByAddressTypeID(int addressTypeID) {
            var query = Search(_entities.AddressTypes, x => x.AddressTypeID == addressTypeID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; AddressType with addressTypeID = '{addressTypeID}' doesn't exist." }); 
            }
        }
