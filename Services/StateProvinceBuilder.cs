...
namespace PersonAPI.Services {
    ...
    public class StateProvinceBuilder : IStateProvinceBuilder {
        ...
        public async Task<BuilderResponse> GetStateProvince_ByStateProvinceID(int stateProvinceID) {
            var query = Search(_entities.StateProvinces, x => x.StateProvinceID == stateProvinceID).Select(ProjectToModel);
            if (query.Any()) {
                return await Task.FromResult(new BuilderResponse{ Model = query.Single() }); 
            }
            else {
                return await Task.FromResult(new BuilderResponse { ValidationMessage = $"Record Not Found; StateProvince with stateProvinceID = '{stateProvinceID}' doesn't exist." }); 
            }
        }
